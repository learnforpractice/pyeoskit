eosio_system_abi_eos = r'''{
    ...
    "actions": [
        ...
        },
        {
            "name": "voteproducer",
            "type": "voteproducer",
            "ricardian_contract": "---\nspec_version: \"0.2.0\"\ntitle: Vote for Block Producers\nsummary: '{{nowrap voter}} votes for {{#if proxy}}the proxy {{nowrap proxy}}{{else}}up to 30 block producer candidates{{/if}}'\nicon: http://127.0.0.1/ricardian_assets/eosio.contracts/icons/voting.png#db28cd3db6e62d4509af3644ce7d377329482a14bb4bfaca2aa5f1400d8e8a84\n---\n\n{{#if proxy}}\n{{voter}} votes for the proxy {{proxy}}.\nAt the time of voting the full weight of voters staked (CPU + NET) tokens will be cast towards each of the producers voted by {{proxy}}.\n{{else}}\n{{voter}} votes for the following block producer candidates:\n\n{{#each producers}}\n  + {{this}}\n{{/each}}\n\nAt the time of voting the full weight of voters staked (CPU + NET) tokens will be cast towards each of the above producers.\n{{/if}}"
        },
        {
            "name": "withdraw",
            "type": "withdraw",
    ...
    "structs": [
        ...
        },
        {
            ...
            "fields": [
                ...
                {
                    "name": "min_bp_staking_amount",
                    "type": "int64"
                },
                {
                    "name": "vote_producer_limit",
                    "type": "uint32"
                },
                {
                    "name": "mini_voting_requirement",
                    "type": "uint32"
                }
            ]
        },
